...
vec3 dither(vec2 pos,vec3 color){
	...
	float bias = -.25; // This variable can be adjusted in range of -1 to 1 for more desirable result(Or maybe as a uniform);
...
void main() {
    ...
    vec3 c = dither(gl_FragCoord.xy, rgb2hsl(color.rgb));
